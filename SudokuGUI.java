...
public class SudokuGUI extends JComponent implements MouseListener, KeyListener {
	...
	private final Image active_cell;
	private final Image solve;
	private final Image notSolvable;
    private final Image border_left;
	private final Image border_right;
	private final Image border_top;
	private final Image border_bottom;
    private final Image corner_left_top;
	private final Image corner_left_bottom;
	private final Image corner_right_top;
	private final Image corner_right_bottom;
    ...
    private final ISudokuSolver solver;
    private int cCol;
    private int cRow;
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 1; i < num; i++) {
            ...
            g.drawLine(num * imSize * i + ins, ins, num * imSize * i + ins, size * imSize + ins);
        }

        g.drawImage(solve, imSize * (size + 1) + ins, imSize * (size / 2) + ins, this);

        if (!canBeSolved)
            g.drawImage(notSolvable, ins + (int) (imSize * ((double) size / 2) - 125), (int) (imSize * ((double) size / 2) + ins - 50), this);
    }
